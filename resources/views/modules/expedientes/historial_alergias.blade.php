...
@section('titulo_pagina', 'Historial Alergias')
@section('titulo_seccion', 'Historial de Alergias: ' . $mascota->nombre)
...
@section('contenido')
    ...
    <div class="card shadow mb-4 border-left-info">
        <div class="card-header py-3 bg-white d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-info">Historial de Alergias (Global de la mascota)</h6>
            <a href="{{ route('expedientes.consultas.alergias.crear', [$mascota->id, $consulta->id]) }}" class="btn btn-sm btn-primary shadow-sm">
                <i class="fas fa-plus fa-sm text-white-50 mr-1"></i> Registrar Nueva Alergia
            </a>
        </div>
        <div class="card-body">
            @if($mascota->alergias->isEmpty())
                <div class="alert alert-info">
                    <i class="fas fa-info-circle mr-2"></i> Esta mascota no tiene alergias registradas.
                </div>
            @else
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($mascota->alergias as $alergia)
                                <tr>
                                    <td class="font-weight-bold">{{ $alergia->sustancia_alergena }}</td>
                                    <td>{{ $alergia->reaccion }}</td>
                                    <td>{{ $alergia->created_at->format('d/m/Y') }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('expedientes.consultas.alergias.delete', [$mascota->id, $consulta->id, $alergia->id]) }}" class="btn btn-danger btn-sm shadow-sm">
